  ...
  ltkTemplate: Handlebars.compile(
	...
	'<span id="loading" style="display:none">loading . . .</span>'+
		'<form enctype="multipart/form-data" method="post"  id="img-form">'+
			...
			'<button id="upload-proxy" type="button" class="action-buttons btn btn-default submit-form-button">+ Image</button>'+
   ...
   render: function() {
	...
	if($("#files")[0].files.length > 0) {
		  ...
		  imgdata = imgdata.replace(/^data:image\/(png|jpeg|gif);base64,/, "");
	...
	handleFileSelect: function(evt) {
		...
		for (var i = 0, f; f = files[i]; i++) {
			...
			reader.onload = (function(theFile) {
				...
				span.innerHTML = ['<img style="display:none" class="thumb" width="100" src="', e.target.result,
								'" title="', escape(theFile.name), '"/>'].join('');
				document.getElementById('list').insertBefore(span, null);
				$("#list").append("<span class='confirm'>Image uploaded!</span>");
